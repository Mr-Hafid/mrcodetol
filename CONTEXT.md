# MrCodeTol Session Runtime

MrCodeTol sessions preserve durable conversational history while assembling the runtime context an agent needs to act correctly in its current environment.
...
A temporary file created under MrCodeTol's shared tool-output directory to retain complete output that was too large for Session history.
...
**MrCodeTol Client**:
The generated Promise and Effect APIs derived from the public `HttpApi`; **Embedded MrCodeTol** shares the Effect API through an in-memory `HttpClient` against the same router and handlers.
...
**Embedded MrCodeTol**:
A scoped in-process host that structurally extends the **MrCodeTol Client**, supplies an in-memory HTTP transport, and exposes additional same-process capabilities directly.
...
- Networked and **Embedded MrCodeTol** use the same **MrCodeTol Client** and preserve the full HTTP encoding, routing, middleware, and decoding boundary; only the `HttpClient` transport differs.
...
- Creating **Embedded MrCodeTol** is scoped. Closing its owning Scope releases the in-process server resources, database resources, registrations, and fibers.
- **Embedded MrCodeTol** exposes shared client capabilities and embedded-only capabilities on one object; consumers do not navigate through a nested `.client` property.
- The beta **MrCodeTol Client** currently uses plural consumer-facing capability groups such as `sessions`; whether the stable Session namespace should instead be singular `session` must be settled before stabilization. Internal server identifiers do not implicitly define public client names.
- Server's concrete `HttpApi` is authoritative for shared **MrCodeTol Client** capabilities. Codegen compiles its Session group directly; the Effect runtime uses an equivalent Protocol-only projection so generated artifacts remain independent of Core and Server.
...
- `@mrcodetol/protocol` owns Session endpoint construction and middleware placement. Server supplies concrete middleware keys to produce the authoritative build-time API; the client projection supplies transport-only keys without importing Core or Server at runtime.
...
- Promise and Effect network clients ship from `@mrcodetol/client` behind isolated root and `/effect` exports. The root has no runtime path to Effect; `/effect` imports only Effect, Schema, and Protocol.
- The Effect-native scoped host belongs to `@mrcodetol/sdk-next`, which will assume the existing `@mrcodetol/sdk` name after legacy consumers migrate. Client remains network-only and SDK depends one-way on Client.
...
- A capability intended for both networked and **Embedded MrCodeTol** belongs in the authoritative public `HttpApi`; embedded-only same-process capabilities extend **Embedded MrCodeTol** separately.
...
- The initial common MrCodeTol Client does not expose server-global event aggregation. `events.subscribe()` is bounded to the connected MrCodeTol instance or workspace; any future cross-instance administrative stream requires a separately designed API.
...
- The stable `sessions.list(...)` design returns a **Page** in both networked and **Embedded MrCodeTol**; embedded execution does not define a separate unbounded array-returning list operation. The beta client currently preserves the existing HTTP `{ data, cursor }` envelope until emitter-level Page projection is implemented.
...
- `sessions.create(...)` accepts an optional `location`. Omission resolves through the connected MrCodeTol instance's default or current location; an explicit value selects a known location. Networked and embedded transports use the same handler semantics.
...
- The **Embedded MrCodeTol** Layer delegates to the same scoped creation path; it does not define a second implementation.
...
- Preserve browser-safe `@mrcodetol/client` and `@mrcodetol/client/effect` bundles through import-boundary tests.
