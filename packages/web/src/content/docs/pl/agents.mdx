...
W MrCodeTol obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
MrCodeTol zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
MrCodeTol ma trzech wbudowanych subagentów: **General**, **Explore** i **Scout**. Przyjrzymy się im poniżej.

---

## Wbudowane

MrCodeTol ma dwa wbudowane agenty główne i trzech wbudowanych subagentów.
...
Agent tylko do odczytu do pracy z zewnętrzną dokumentacją i badaniem zależności. Używaj go, gdy chcesz sklonować repozytorium zależności do zarządzanej pamięci podręcznej MrCodeTol, przejrzeć kod źródłowy biblioteki albo porównać lokalny kod z implementacjami upstream bez modyfikowania swojego workspace.
...
Jeśli nie zostanie skonfigurowana, MrCodeTol używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji MrCodeTol, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji MrCodeTol w `provider/model-id`. Na przykład, [MrCodeTol Zen](/docs/zen), użyj `opencode/gpt-5.1-codex` dla GPT 5.1 Codex.
