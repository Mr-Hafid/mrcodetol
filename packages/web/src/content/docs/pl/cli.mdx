---
title: CLI
description: Opcje i polecenia wiersza poleceń MrCodeTol.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CLI MrCodeTol domyślnie uruchamia [TUI](/docs/tui), gdy wywołasz je bez argumentów.

```bash
opencode
```

CLI obsługuje też polecenia opisane na tej stronie, więc możesz używać MrCodeTol programowo.
...
Uruchamia interfejs terminalowy (TUI) MrCodeTol.
...
Interfejs CLI MrCodeTol zawiera także dodatkowe polecenia.

---

### agent

Zarządzaj agentami MrCodeTol.
...
Dołącz terminal do już działającego serwera backendu MrCodeTol uruchomionego za pomocą `serve` lub `web`.

```bash
opencode attach [url]
```

Pozwala to połączyć TUI ze zdalnym backendem MrCodeTol. Na przykład:
...
MrCodeTol integruje się z wieloma dostawcami. Możesz użyć `opencode auth login`, aby skonfigurować klucz API dla dowolnego obsługiwanego dostawcy. Klucze są bezpiecznie przechowywane w `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

Podczas uruchamiania MrCodeTol ładuje dostawców z pliku poświadczeń, a także ze zmiennych środowiskowych lub pliku `.env` w Twoim projekcie.
...
Użyj flagi `--refresh`, aby zaktualizować listę modeli w pamięci podręcznej. Jest to przydatne, gdy dostawca dodał nowe modele, które chcesz zobaczyć w MrCodeTol.
...
Uruchom MrCodeTol w trybie nieinteraktywnym, przekazując bezpośrednio prompt.
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Dołącz do działającego serwera MrCodeTol (np. http://localhost:4096)                                   |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Hasło uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_PASSWORD`)                            |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nazwa użytkownika uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_USERNAME` lub `opencode`) |
...
Uruchom serwer MrCodeTol (bez interfejsu) w celu uzyskania dostępu do API. Pełny opis API HTTP znajduje się w [dokumentacji serwera](/docs/server).

```bash
opencode serve
```

Uruchamia to serwer HTTP, który zapewnia dostęp do API MrCodeTol bez interfejsu TUI. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `opencode`).
...
Lista wszystkich sesji MrCodeTol.
...
Pokaż statystyki wykorzystania tokenów i sesji MrCodeTol.
...
Importuj dane sesji z pliku JSON lub adresu URL udostępniania MrCodeTol.
...
Uruchom serwer MrCodeTol z interfejsem internetowym.

```bash
opencode web
```

Uruchamia to serwer HTTP i udostępnia MrCodeTol przez interfejs przeglądarkowy. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `opencode`).
...
Odinstaluj MrCodeTol i usuń wszystkie powiązane pliki.
...
Aktualizuj MrCodeTol do najnowszej lub określonej wersji.
...
Interfejs CLI MrCodeTol przyjmuje następujące flagi globalne dla każdego polecenia.
...
MrCodeTol można skonfigurować za pomocą zmiennych środowiskowych.
