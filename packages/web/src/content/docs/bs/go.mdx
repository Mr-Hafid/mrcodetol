...
MrCodeTol Go je povoljna pretplata — **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno** — koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

Go radi kao bilo koji drugi provajder u MrCodeTol-u. Pretplatite se na MrCodeTol Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili MrCodeTol.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa MrCodeTol-om.
:::
...
MrCodeTol Go vam daje pristup ovim modelima za **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno**.

---

## Kako funkcioniše

MrCodeTol Go radi kao bilo koji drugi provajder u MrCodeTol-u.

1. Prijavite se na **<a href={console}>MrCodeTol Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `MrCodeTol Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na MrCodeTol Go.
:::
...
MrCodeTol Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj MrCodeTol konfiguraciji
...
Napravili smo MrCodeTol Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz MrCodeTol koristite i bilo kojeg drugog provajdera.
