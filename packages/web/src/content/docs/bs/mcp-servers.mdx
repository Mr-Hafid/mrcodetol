...
Možete dodati vanjske alate u MrCodeTol koristeći _Model Context Protocol_, ili MCP. MrCodeTol podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [MrCodeTol Config](https://opencode.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
MrCodeTol automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, MrCodeTol će:
...
Ako server zahtijeva autentifikaciju, MrCodeTol će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `opencode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, MrCodeTol sigurno čuva tokene u `~/.local/share/opencode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u MrCodeTol, zajedno s ugrađenim alatima. Možete njima upravljati kroz MrCodeTol konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete MrCodeTol sa vašim Sentry nalogom.
