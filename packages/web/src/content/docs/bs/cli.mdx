---
title: CLI
description: MrCodeTol CLI opcije i naredbe.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

MrCodeTol CLI po defaultu pokreće [TUI](/docs/tui) kada se pokrene bez ikakvih argumenata.

```bash
opencode
```

Ali takođe prihvata naredbe kao što je dokumentovano na ovoj stranici. Ovo vam omogućava programsku interakciju sa MrCodeTol.
...
Pokrenite MrCodeTol terminalski korisnički interfejs.
...
MrCodeTol CLI takođe ima sljedeće naredbe.

---

### agent

Upravljajte MrCodeTol agentima.
...
Priključite terminal na već pokrenut MrCodeTol backend server pokrenut putem `serve` ili `web` naredbi.

```bash
opencode attach [url]
```

Ovo omogućava korištenje TUI-ja sa udaljenim MrCodeTol backend-om. Na primjer:
...
MrCodeTol pokreće lista provajdera na [Models.dev](https://models.dev), tako da možete koristiti `opencode auth login` da konfigurirate API ključeve za bilo kojeg provajdera kojeg želite koristiti. Ovo je pohranjeno u `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

Kada se MrCodeTol pokrene, učitava dobavljače iz datoteke vjerodajnica. I ako postoje neki ključevi definirani u vašim okruženjima ili `.env` fajl u vašem projektu.
...
Koristite `--refresh` zastavicu da ažurirate keširanu listu modela. Ovo je korisno kada su novi modeli dodani provajderu i želite da ih vidite u MrCodeTol.
...
Pokrenite MrCodeTol u neinteraktivnom modu tako što ćete direktno proslijediti prompt.
...
| <nobr><code>{"--attach"}</code></nobr>   |        | Priključite na pokrenuti MrCodeTol server (npr. http://localhost:4096)                         |
| <nobr><code>{"--password"}</code></nobr> | `-p`   | Lozinka za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_PASSWORD`)                       |
| <nobr><code>{"--username"}</code></nobr> | `-u`   | Korisničko ime za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_USERNAME` ili `opencode`) |
...
Pokrenite MrCodeTol headless server za API pristup. Pogledajte [server docs](/docs/server) za kompletan HTTP interfejs.

```bash
opencode serve
```

Ovo pokreće HTTP server koji pruža API pristup funkcionalnosti MrCodeTol-a bez TUI interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `opencode`).
...
Upravljajte MrCodeTol sesijama.
...
Prikaži statistiku upotrebe tokena i troškova za vaše MrCodeTol sesije.
...
Uvezite podatke sesije iz JSON datoteke ili MrCodeTol dijeljenog URL-a.

```bash
opencode import <file>
```

Možete uvesti iz lokalne datoteke ili MrCodeTol dijeljenog URL-a.
...
Pokrenite MrCodeTol headless server sa web interfejsom.

```bash
opencode web
```

Ovo pokreće HTTP server i otvara web pretraživač za pristup MrCodeTol-u preko web interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `opencode`).
...
Deinstalirajte MrCodeTol i uklonite sve povezane datoteke.
...
Ažurira MrCodeTol na najnoviju verziju ili određenu verziju.
...
MrCodeTol CLI prihvata sljedeće globalne zastavice.
...
MrCodeTol se može konfigurirati pomoću varijabli okruženja.
