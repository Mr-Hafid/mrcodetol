---
title: サーバー
description: HTTP 経由で MrCodeTol サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`opencode serve` コマンドは、MrCodeTol クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
:::tip
MrCodeTol サーバーを使用して、プログラムで MrCodeTol と対話します。
:::

`opencode serve` を実行してスタンドアロンサーバーを起動できます。すでに MrCodeTol TUI を実行している場合でも、`opencode serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、MrCodeTol [IDE](/docs/ide) プラグインによって使用されます。
