---
title: LSPサーバー
description: MrCodeTol は LSP サーバーと統合します。
---

MrCodeTol は Language Server Protocol (LSP) サーバーと統合し、診断を agent へのフィードバックとして使用できます。

---

## 組み込み

MrCodeTol には、一般的な言語用のいくつかの組み込み LSP サーバーが付属しています。
...
LSP が有効で MrCodeTol がファイルを開くと、次のようになります。
...
MrCodeTol 設定の `lsp` セクションを通じて LSP サーバーを有効化し、カスタマイズできます。
