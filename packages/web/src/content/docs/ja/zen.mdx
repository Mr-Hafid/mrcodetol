---
title: MrCodeTol Zen
description: MrCodeTol によって提供されるモデルの厳選されたリスト。
---
...
MrCodeTol Zen は、MrCodeTol チームが提供する、テスト済みかつ検証済みのモデルの一覧です。

Zen は MrCodeTol のほかのプロバイダーと同じように動作します。MrCodeTol Zen にログインして API キーを取得できます。これは **完全にオプション** であり、MrCodeTol を使うために必須ではありません。
...
:::tip
MrCodeTol でうまく動作する、厳選したモデルとプロバイダーの組み合わせをテストしました。
:::
...
MrCodeTol Zen は、これらのモデルにアクセスできる AI ゲートウェイです。

---

## 仕組み

MrCodeTol Zen は、MrCodeTol のほかのプロバイダーと同じように動作します。

1. **<a href={console}>MrCodeTol Zen</a>** にサインインし、請求情報を追加して、API キーをコピーします。
2. TUI で `/connect` コマンドを実行し、MrCodeTol Zen を選んで API キーを貼り付けます。
...
MrCodeTol 設定で使う [model id](/docs/config/#models) は `opencode/<model-id>` 形式です。たとえば、GPT 5.5 では設定に `opencode/gpt-5.5` を使用します。
...
使用履歴に Haiku、Nano、Flash などの[低コストモデル](/docs/config/#models)が表示されることがあります。MrCodeTol はこれらのモデルをセッションタイトルの生成に使用します。
...
- DeepSeek V4 Flash Free は期間限定で MrCodeTol で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- MiMo-V2.5 Free は期間限定で MrCodeTol で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Hy3 Free は期間限定で MrCodeTol で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Laguna S 2.1 Free は期間限定で MrCodeTol で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Nemotron 3 Ultra Free は期間限定で MrCodeTol で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Nemotron 3.5 Lightning Free は期間限定で MrCodeTol で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Big Pickle はステルスモデルで、期間限定で MrCodeTol で無料提供されています。チームはこの期間中にフィードバックを集め、モデルを改善しています。
...
私たちが MrCodeTol Zen を作った目的は次のとおりです。
...
4. ほかのコーディングエージェントでも使えるようにして **no lock-in** を実現し、さらに MrCodeTol では常にほかのプロバイダーも使えるようにすること。
