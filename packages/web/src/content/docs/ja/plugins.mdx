---
title: プラグイン
description: MrCodeTol を拡張する独自のプラグインを作成します。
---

プラグインを使用すると、さまざまなイベントにフックして動作をカスタマイズすることで MrCodeTol を拡張できます。プラグインを作成して、新しい機能を追加したり、外部サービスと統合したり、MrCodeTol のデフォルトの動作を変更したりできます。
...
MrCodeTol は起動時に `bun install` を実行してこれらをインストールします。プラグインとツールはそれらをインポートできるようになります。
...
- `client`: AI と対話するための MrCodeTol SDK クライアント。
...
ここでは、MrCodeTol を拡張するために使用できるプラグインの例をいくつか示します。
...
MrCodeTol が `.env` ファイルを読み取らないようにします。
...
プラグインは MrCodeTol にカスタムツールを追加することもできます。
...
`tool` ヘルパーは、MrCodeTol が呼び出すことができるカスタムツールを作成します。 Zod スキーマ関数を受け取り、次のようなツール定義を返します。

- `description`: ツールの機能
- `args`: ツールの引数の Zod スキーマ
- `execute`: ツールが呼び出されたときに実行される関数

カスタムツールは、組み込みツールと並行して MrCodeTol で使用できます。
