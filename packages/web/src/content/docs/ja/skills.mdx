...
エージェントスキルにより、MrCodeTol はリポジトリまたはホームディレクトリから再利用可能な命令を検出できます。
...
プロジェクトのローカルパスの場合、MrCodeTol は現在の作業ディレクトリから git ワークツリーに到達するまで進みます。
...
MrCodeTol では、`skill` ツールの説明に利用可能なスキルがリストされています。
