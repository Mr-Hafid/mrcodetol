...
O MrCodeTol Go é uma assinatura de baixo custo — **US$ 5 no seu primeiro mês**, depois **US$ 10/mês** — que oferece acesso confiável a modelos abertos de programação populares.

O Go funciona como qualquer outro provedor no MrCodeTol. Você assina o MrCodeTol Go e
...
O MrCodeTol Go lhe dá acesso a esses modelos por **US$ 5 no seu primeiro mês**, depois **US$ 10/mês**.

---

## Como funciona

O MrCodeTol Go funciona como qualquer outro provedor no MrCodeTol.

1. Você entra no **<a href={console}>MrCodeTol Zen</a>**, assina o Go e
   copia a sua chave de API.
2. Você executa o comando `/connect` na TUI, seleciona `MrCodeTol Go` e cola
   a sua chave de API.
3. Execute `/models` na TUI para ver a lista de modelos disponíveis através do Go.

:::note
Apenas um membro por workspace pode assinar o MrCodeTol Go.
:::
...
O MrCodeTol Go inclui os seguintes limites:
...
O [ID do modelo](/docs/config/#models) na sua configuração do MrCodeTol
...
4. Não ter **nenhuma restrição de fornecedor (lock-in)**, permitindo que você use qualquer outro provedor com o MrCodeTol também.
