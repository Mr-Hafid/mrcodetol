---
title: Providers
description: Using any LLM provider in MrCodeTol.
---

import config from "../../../config.mjs"
export const console = config.console

MrCodeTol uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your MrCodeTol config.
...
You can customize the providers through the `provider` section in your MrCodeTol
config.
...
MrCodeTol Zen is a list of models provided by the MrCodeTol team that have been
tested and verified to work well with MrCodeTol. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with MrCodeTol Zen.
:::

1. Run the `/connect` command in the TUI, select `MrCodeTol Zen`, and head to [opencode.ai/auth](https://opencode.ai/zen).
...
It works like any other provider in MrCodeTol and is completely optional to use.

---

## MrCodeTol Go

MrCodeTol Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the MrCodeTol team that have been
tested and verified to work well with MrCodeTol.

1. Run the `/connect` command in the TUI, select `MrCodeTol Go`, and head to [opencode.ai/auth](https://opencode.ai/zen).
...
To use Amazon Bedrock with MrCodeTol:
...
MrCodeTol. Anthropic explicitly prohibits this.

Previous versions of MrCodeTol came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in MrCodeTol with zero setup:
...
MrCodeTol supports two authentication methods:

- **OAuth (Recommended)** — Sign in to your DigitalOcean account; MrCodeTol uses your DigitalOcean API token directly for inference and discovers your Inference Routers.
...
3. Your browser opens to authorize MrCodeTol. Sign in and approve.

   :::note
   MrCodeTol requests `genai:read` and `inference:query` OAuth scopes. Your DigitalOcean API token is used directly for inference — no separate Model Access Key is created.
   :::
...
Inference Routers let you define a routing policy across multiple models — picking the cheapest, fastest, or most appropriate model per request based on the task. After OAuth, MrCodeTol surfaces each router as `router:<router-name>` in the model picker.

Selecting a router model is a drop-in replacement for any other model — MrCodeTol forwards your request and DigitalOcean picks the underlying model based on your router's policy. Learn more about [Inference Routers](https://docs.digitalocean.com/products/inference/how-to/use-inference-router/)
...
GitLab Duo support in MrCodeTol is experimental. Features, configuration, and
behavior may change in future releases.
:::

MrCodeTol integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
MrCodeTol uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock MrCodeTol
...
When a `duo-workflow-*` model is selected, MrCodeTol will:
...
4. Route tool execution requests through MrCodeTol's permission-gated tool system
...
To use GMI Cloud with MrCodeTol:
...
To use Google Vertex AI with MrCodeTol:
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin to automatically log each MrCodeTol conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each MrCodeTol conversation listed as a separate session.
...
:::tip
Ollama can automatically configure itself for MrCodeTol. See the [Ollama integration docs](https://docs.ollama.com/integrations/opencode) for details.
:::
...
To use Ollama Cloud with MrCodeTol:
...
3. Copy the API key for use in MrCodeTol.
...
6. **Important**: Before using cloud models in MrCodeTol, you must pull the model information locally:
...
MrCodeTol Zen is a list of tested and verified models provided by the MrCodeTol team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>MrCodeTol Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **MrCodeTol Zen**.

   ```txt
   /connect
   ```

3. Enter your MrCodeTol API key.
...
5. Complete sign-in in the browser that opens. MrCodeTol captures the OAuth callback automatically and stores the token — no copy/paste needed.
...
2. Select **SuperGrok Subscription**. MrCodeTol opens xAI's verification link with the user code pre-populated when supported.

3. Approve the consent screen. If xAI asks for a code, enter the user code displayed by MrCodeTol. MrCodeTol polls xAI's token endpoint and stores the resulting OAuth tokens once you approve.
...
MrCodeTol refreshes the OAuth access token automatically. Any Grok or X Premium plan that includes Grok API access works; you do not need a separate `XAI_API_KEY`.
...
The `limit` fields allow MrCodeTol to understand how much context you have left. Standard providers pull these from models.dev automatically.
