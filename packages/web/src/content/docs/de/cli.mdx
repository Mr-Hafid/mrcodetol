---
title: CLI
description: MrCodeTol CLI Optionen und Befehle.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Der MrCodeTol CLI startet standardmäßig das [TUI](/docs/tui), wenn er ohne Argumente ausgeführt wird.

```bash
opencode
```

Es akzeptiert aber auch Befehle, wie auf dieser Seite dokumentiert. Dadurch können Sie programmgesteuert mit MrCodeTol interagieren.
...
Starten Sie die Terminalbenutzeroberfläche MrCodeTol.
...
Der MrCodeTol CLI verfügt außerdem über die folgenden Befehle.

---

### agent

Agenten für MrCodeTol verwalten.
...
Schließen Sie ein Terminal an einen bereits laufenden MrCodeTol-Backend-Server an, der über die Befehle `serve` oder `web` gestartet wurde.

```bash
opencode attach [url]
```

Dies ermöglicht die Verwendung von TUI mit einem Remote-Backend MrCodeTol. Zum Beispiel:
...
MrCodeTol wird von der Anbieterliste bei [Models.dev](https://models.dev) unterstützt, sodass Sie `opencode auth login` verwenden können, um API-Schlüssel für jeden Anbieter zu konfigurieren, den Sie verwenden möchten. Dies wird in `~/.local/share/opencode/auth.json` gespeichert.

```bash
opencode auth login
```

Wenn MrCodeTol startet, lädt es den Anbieter aus der Anmeldeinformationsdatei. Und ob in Ihren Umgebungen Schlüssel sind oder in Ihrem Projekt eine `.env`-Datei vorhanden ist.
...
Verwenden Sie das Flag `--refresh`, um die zwischengespeicherte Modellliste zu aktualisieren. Dies ist nützlich, wenn einem Anbieter neue Modelle hinzugefügt wurden und Sie diese in MrCodeTol sehen möchten.
...
Führen Sie MrCodeTol im nicht interaktiven Modus aus, indem Sie direkt eine Eingabeaufforderung übergeben.
...
| <nobr><code>{"--attach"}</code></nobr>   |      | An einen laufenden MrCodeTol-Server anschließen (z.B. http://localhost:4096)                         |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Basic-Auth-Passwort (standardmäßig `OPENCODE_SERVER_PASSWORD`)                                      |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Basic-Auth-Benutzername (standardmäßig `OPENCODE_SERVER_USERNAME` oder `opencode`)                  |
...
Starten Sie einen Headless-MrCodeTol-Server für den API-Zugriff. Sehen Sie sich die [Server-Dokumentation](/docs/server) für die vollständige HTTP-Schnittstelle an.

```bash
opencode serve
```

Dadurch wird ein HTTP-Server gestartet, der API-Zugriff auf MrCodeTol-Funktionalität ohne die TUI-Schnittstelle bietet. Legen Sie `OPENCODE_SERVER_PASSWORD` fest, um die HTTP-Basisauthentifizierung zu aktivieren (Benutzername ist standardmäßig `opencode`).
...
Listen Sie alle MrCodeTol-Sitzungen auf.
...
Zeigen Sie Token-Nutzungs- und Kostenstatistiken für Ihre MrCodeTol-Sitzungen an.
...
Importieren Sie Sitzungsdaten aus einer JSON-Datei oder einer MrCodeTol-Freigabe-URL.

```bash
opencode import <file>
```

Sie können aus einer lokalen Datei oder einer MrCodeTol-Freigabe-URL importieren.
...
Starten Sie einen Headless-MrCodeTol-Server mit einer Weboberfläche.

```bash
opencode web
```

Dadurch wird ein HTTP-Server gestartet und ein Webbrowser geöffnet, um über eine Webschnittstelle auf MrCodeTol zuzugreifen. Legen Sie `OPENCODE_SERVER_PASSWORD` fest, um die HTTP-Basisauthentifizierung zu aktivieren (Benutzername ist standardmäßig `opencode`).
...
Deinstallieren Sie MrCodeTol und entfernen Sie alle zugehörigen Dateien.
...
Aktualisiert MrCodeTol auf die neueste Version oder eine bestimmte Version.
...
Der MrCodeTol CLI akzeptiert die folgenden globalen Flags.
...
MrCodeTol kann mithilfe von Umgebungsvariablen konfiguriert werden.
