---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um MrCodeTol zu erweitern.
---

Mit Plugins können Sie MrCodeTol erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von MrCodeTol zu ändern.
...
MrCodeTol führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein MrCodeTol-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie MrCodeTol erweitern können.
...
:::note
Wenn Sie die MrCodeTol-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass MrCodeTol `.env`-Dateien liest:
...
Plugins können MrCodeTol auch benutzerdefinierte Tools hinzufügen:

```ts title=".opencode/plugins/custom-tools.ts"
import { type Plugin, tool } from "@mrcodetol/plugin"
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das MrCodeTol aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für MrCodeTol zur Verfügung.
