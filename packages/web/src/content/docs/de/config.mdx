---
title: Konfiguration
description: Verwenden Sie die Konfiguration MrCodeTol JSON.
---

Sie können MrCodeTol mithilfe einer JSON-Konfigurationsdatei konfigurieren.

---

## Format

MrCodeTol unterstützt die Formate **JSON** und **JSONC** (JSON mit Kommentaren).
...
Platzieren Sie Ihre globale MrCodeTol-Konfiguration in `~/.config/opencode/opencode.json`. Verwenden Sie die globale Konfiguration für benutzerweite Einstellungen wie Themen, Anbieter oder Tastenkombinationen.
...
Wenn MrCodeTol gestartet wird, sucht es im aktuellen Verzeichnis nach einer Konfigurationsdatei oder durchsucht das nächstgelegene Git-Verzeichnis.
...
- `mdns` – mDNS-Diensterkennung aktivieren. Dadurch können andere Geräte im Netzwerk Ihres MrCodeTol-Servers erkannt werden.
...
Sie können die Anbieter und Modelle, die Sie in Ihrer MrCodeTol-Konfiguration verwenden möchten, über die Optionen `provider`, `model` und `small_model` konfigurieren.
...
Die Option `small_model` konfiguriert ein separates Modell für einfache Aufgaben wie die Titelgenerierung. Standardmäßig versucht MrCodeTol, ein günstigeres Modell zu verwenden, sofern eines bei Ihrem Anbieter verfügbar ist, andernfalls greift es auf Ihr Hauptmodell zurück.
...
Sie können das Thema, das Sie in Ihrer MrCodeTol-Konfiguration verwenden möchten, über die Option `theme` konfigurieren.
...
Der Standardagent muss ein Primäragent (kein Subagent) sein. Dies kann ein integrierter Agent wie `"build"` oder `"plan"` oder ein von Ihnen definierter [custom agent](/docs/agents) sein. Wenn der angegebene Agent nicht existiert oder ein Subagent ist, greift MrCodeTol mit einer Warnung auf `"build"` zurück.
...
MrCodeTol lädt beim Start automatisch alle neuen Updates herunter. Sie können dies mit der Option `autoupdate` deaktivieren.
...
MrCodeTol erlaubt standardmäßig alle Vorgänge, ohne dass eine ausdrückliche Genehmigung erforderlich ist. Sie können dies mit der Option `permission` ändern.
...
[Plugins](/docs/plugins) erweitert MrCodeTol mit benutzerdefinierten Tools, Hooks und Integrationen.
...
Dies ist nützlich, wenn Sie MrCodeTol darauf beschränken möchten, nur bestimmte Anbieter zu verwenden, anstatt sie einzeln zu deaktivieren.
