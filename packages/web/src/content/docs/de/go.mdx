...
MrCodeTol Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

Go funktioniert wie jeder andere Provider in MrCodeTol. Du abonnierst MrCodeTol Go und
erhältst deinen API-Key. Es ist **völlig optional** und du musst es nicht nutzen, um
MrCodeTol zu verwenden.
...
MrCodeTol Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

MrCodeTol Go funktioniert wie jeder andere Provider in MrCodeTol.

1. Du meldest dich bei **<a href={console}>MrCodeTol Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `MrCodeTol Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann MrCodeTol Go abonnieren.
:::
...
MrCodeTol Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner MrCodeTol Config verwendet das Format `opencode-go/<model-id>`. Für Kimi K3 würdest du beispielsweise `opencode-go/kimi-k3` in deiner Config verwenden.
...
Wir haben MrCodeTol Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit MrCodeTol zu nutzen.
