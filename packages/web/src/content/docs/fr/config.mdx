---
title: Configuration
description: Utilisation de la configuration JSON MrCodeTol.
---

Vous pouvez configurer MrCodeTol à l'aide d'un fichier de configuration JSON.

---

## Format

MrCodeTol prend en charge les formats **JSON** et **JSONC** (JSON avec commentaires).
...
Placez votre configuration globale MrCodeTol dans `~/.config/opencode/opencode.json`. Utilisez la configuration globale pour les préférences de l'utilisateur telles que les fournisseurs, les modèles et les autorisations.
...
Lorsque MrCodeTol démarre, il recherche un fichier de configuration dans le répertoire actuel ou remonte jusqu'au répertoire Git le plus proche.
...
- `mdns` - Activer la découverte du service mDNS. Cela permet à d'autres appareils du réseau de découvrir votre serveur MrCodeTol.
...
Vous pouvez configurer les fournisseurs et les modèles que vous souhaitez utiliser dans votre configuration MrCodeTol via les options `provider`, `model` et `small_model`.
...
L'option `small_model` configure un modèle distinct pour les tâches légères comme la génération de titres. Par défaut, MrCodeTol essaie d'utiliser un modèle moins cher s'il est disponible auprès de votre fournisseur, sinon il revient à votre modèle principal.
...
L'agent par défaut doit être un agent principal (et non un sous-agent). Il peut s'agir d'un agent intégré tel que `"build"` ou `"plan"`, ou d'un [agent personnalisé](/docs/agents) que vous avez défini. Si l'agent spécifié n'existe pas ou est un sous-agent, MrCodeTol reviendra à `"build"` avec un avertissement.
...
MrCodeTol téléchargera automatiquement toutes les nouvelles mises à jour au démarrage. Vous pouvez désactiver cela avec l'option `autoupdate`.
...
[Plugins](/docs/plugins) étendent MrCodeTol avec des outils, des hooks et des intégrations personnalisés.
...
Ceci est utile lorsque vous souhaitez restreindre MrCodeTol à l'utilisation de fournisseurs spécifiques plutôt que de les désactiver un par un.
