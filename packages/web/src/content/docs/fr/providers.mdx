---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en MrCodeTol.
---

import config from "../../../../config.mjs"
export const console = config.console

MrCodeTol utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration MrCodeTol.
...
Vous pouvez personnaliser les fournisseurs via la section `provider` de votre MrCodeTol
configuration.
...
MrCodeTol Zen est une liste de modèles fournis par l'équipe MrCodeTol qui ont été
testé et vérifié pour fonctionner correctement avec MrCodeTol. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par MrCodeTol Zen.
:::
...
Il fonctionne comme n’importe quel autre fournisseur dans MrCodeTol et son utilisation est totalement facultative.

---

## MrCodeTol Go

MrCodeTol Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe MrCodeTol qui ont été
testé et vérifié pour fonctionner correctement avec MrCodeTol.

1. Exécutez la commande `/connect` dans le TUI, sélectionnez `MrCodeTol Go` et rendez-vous sur [opencode.ai/auth](https://opencode.ai/zen).
...
Pour utiliser Amazon Bedrock avec MrCodeTol :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans MrCodeTol n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
MrCodeTol utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller MrCodeTol
...
Pour utiliser Google Vertex AI avec MrCodeTol :
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) pour enregistrer automatiquement chaque conversation MrCodeTol en tant que session dans Helicone.
...
Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation MrCodeTol répertoriée comme une session distincte.
...
:::tip
Ollama peut se configurer automatiquement pour MrCodeTol. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/opencode) pour plus de détails.
:::
...
Pour utiliser Ollama Cloud avec MrCodeTol :
...
3. Copiez la clé API à utiliser dans MrCodeTol.
...
6. **Important** : Avant d'utiliser des modèles cloud dans MrCodeTol, vous devez extraire les informations du modèle localement :
...
MrCodeTol Zen est une liste de modèles testés et vérifiés fournie par l'équipe MrCodeTol. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>MrCodeTol Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **MrCodeTol Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé MrCodeTol API.
...
Les champs `limit` permettent à MrCodeTol de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
