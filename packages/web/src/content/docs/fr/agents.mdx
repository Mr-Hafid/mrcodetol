...
Il existe deux types d'agents dans MrCodeTol ; agents primaires et sous-agents.
...
MrCodeTol est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
MrCodeTol est livré avec trois sous-agents intégrés, **General**, **Explore** et **Scout**. Nous les verrons ci-dessous.

---

## Agents intégrés

MrCodeTol est livré avec deux agents primaires intégrés et trois sous-agents intégrés.
...
Si aucune température n'est spécifiée, MrCodeTol utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale MrCodeTol et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration MrCodeTol utilise le format `provider/model-id`. Par exemple, si vous utilisez [MrCodeTol Zen](/docs/zen), vous utiliserez `opencode/gpt-5.1-codex` pour GPT 5.1 Codex.
