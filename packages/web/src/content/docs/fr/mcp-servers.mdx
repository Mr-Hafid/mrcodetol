...
Vous pouvez ajouter des outils externes à MrCodeTol à l'aide du _Model Context Protocol_ ou MCP. MrCodeTol prend en charge les serveurs locaux et distants.
...
Vous pouvez définir des serveurs MCP dans votre [MrCodeTol Config](https://opencode.ai/docs/config/) sous `mcp`. Ajoutez chaque MCP avec un nom unique. Vous pouvez faire référence à ce MCP par son nom lorsque vous demandez le LLM.
...
MrCodeTol gère automatiquement l'authentification OAuth pour les serveurs MCP distants. Lorsqu'un serveur nécessite une authentification, MrCodeTol :
...
Si le serveur nécessite une authentification, MrCodeTol vous demandera de vous authentifier lorsque vous essayez de l'utiliser pour la première fois. Sinon, vous pouvez [déclencher manuellement le flux](#authenticating) avec `opencode mcp auth <server-name>`.
...
La commande `mcp auth` ouvrira votre navigateur pour autorisation. Après votre autorisation, MrCodeTol stockera les jetons en toute sécurité dans `~/.local/share/opencode/mcp-auth.json`.
...
Vos MCP sont disponibles sous forme d'outils dans MrCodeTol, aux côtés des outils intégrés. Vous pouvez donc les gérer via la configuration MrCodeTol comme n'importe quel autre outil.
...
Cela ouvrira une fenêtre de navigateur pour terminer le flux OAuth et connecter MrCodeTol à votre compte Sentry.
