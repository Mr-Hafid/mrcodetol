---
title: Zen
description: Liste organisée de modèles fournis par MrCodeTol.
---
...
MrCodeTol Zen est une liste de modèles testés et vérifiés fournie par l'équipe MrCodeTol.

Zen fonctionne comme n'importe quel autre fournisseur dans MrCodeTol. Vous vous connectez à MrCodeTol Zen et obtenez votre clé API. C'est **entièrement facultatif** et vous n'avez pas besoin de l'utiliser pour utiliser MrCodeTol.
...
:::tip
Nous avons testé un groupe sélectionné de modèles et de fournisseurs qui fonctionnent bien avec MrCodeTol.
:::
...
MrCodeTol Zen est une passerelle AI qui vous donne accès à ces modèles.

---

## Comment ça marche

MrCodeTol Zen fonctionne comme n'importe quel autre fournisseur dans MrCodeTol.

1. Vous vous connectez à **<a href={console}>MrCodeTol Zen</a>**, ajoutez vos informations de facturation et copiez votre clé API.
2. Vous exécutez la commande `/connect` dans le TUI, sélectionnez MrCodeTol Zen et collez votre clé API.
...
Le [model id](/docs/config/#models) dans votre configuration MrCodeTol utilise le format `opencode/<model-id>`. Par exemple, pour GPT 5.5, vous utiliseriez `opencode/gpt-5.5` dans votre configuration.
...
Vous remarquerez peut-être des [modèles à faible coût](/docs/config/#models), tels que Haiku, Nano ou Flash, dans votre historique d'utilisation. MrCodeTol utilise ces modèles pour générer les titres des sessions.
...
- DeepSeek V4 Flash Free est disponible sur MrCodeTol pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
- MiMo-V2.5 Free est disponible sur MrCodeTol pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
- Hy3 Free est disponible sur MrCodeTol pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
- Laguna S 2.1 Free est disponible sur MrCodeTol pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
- Nemotron 3 Ultra Free est disponible sur MrCodeTol pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
- Nemotron 3.5 Lightning Free est disponible sur MrCodeTol pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
- Big Pickle est un modèle stealth gratuit sur MrCodeTol pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
...
Nous avons créé MrCodeTol Zen pour :
...
4. N'avoir **aucun lock-in** en vous permettant de l'utiliser avec n'importe quel autre agent de codage. Et toujours vous permettre d'utiliser n'importe quel autre fournisseur avec MrCodeTol également.
