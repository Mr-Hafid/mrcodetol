---
title: TUI
description: Utilisation de l’interface utilisateur du terminal MrCodeTol.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

MrCodeTol fournit une interface de terminal interactif ou TUI pour travailler sur vos projets avec un LLM.

L’exécution de MrCodeTol démarre le TUI pour le répertoire actuel.
...
Lorsque vous utilisez le MrCodeTol TUI, vous pouvez taper `/` suivi d'un nom de commande pour exécuter rapidement des actions. Par exemple:
...
Ajoutez un fournisseur à MrCodeTol. Vous permet de sélectionner parmi les fournisseurs disponibles et d’ajouter leurs clés API.
...
Quittez MrCodeTol. *Alias* : `/quit`, `/q`
...
- `leader_timeout` - Contrôle la durée pendant laquelle MrCodeTol attend après la leader key. Valeur par défaut : `2000`.
