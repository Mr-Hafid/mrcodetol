...
MrCodeTol Go est un abonnement à bas coût — **5 $ pour votre premier mois**, puis **10 $/mois** — qui vous donne un accès fiable aux modèles de codage ouverts populaires.

Go fonctionne comme n'importe quel autre fournisseur dans MrCodeTol. Vous vous abonnez à MrCodeTol Go et obtenez votre clé d'API. C'est **totalement facultatif** et vous n'avez pas besoin de l'utiliser pour utiliser MrCodeTol.
...
MrCodeTol Go vous donne accès à ces modèles pour **5 $ pour votre premier mois**, puis **10 $/mois**.

---

## Comment ça marche

MrCodeTol Go fonctionne comme n'importe quel autre fournisseur dans MrCodeTol.

1. Vous vous connectez à **<a href={console}>MrCodeTol Zen</a>**, vous vous abonnez à Go et copiez votre clé d'API.
2. Vous exécutez la commande `/connect` dans la TUI, sélectionnez `MrCodeTol Go` et collez votre clé d'API.
3. Exécutez `/models` dans la TUI pour voir la liste des modèles disponibles via Go.

:::note
Un seul membre par espace de travail peut s'abonner à MrCodeTol Go.
:::
...
MrCodeTol Go inclut les limites suivantes :
...
L'[ID de modèle](/docs/config/#models) dans votre configuration MrCodeTol utilise le format `opencode-go/<model-id>`. Par exemple, pour Kimi K3, vous utiliseriez `opencode-go/kimi-k3` dans votre configuration.
...
Nous avons créé MrCodeTol Go pour :
...
4. N'avoir **aucun verrouillage exclusif** en vous permettant d'utiliser n'importe quel autre fournisseur avec MrCodeTol également.
