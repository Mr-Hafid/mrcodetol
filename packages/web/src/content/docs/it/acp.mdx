---
title: Supporto ACP
description: Usa MrCodeTol in qualunque editor compatibile con ACP.
---

MrCodeTol supporta [Agent Client Protocol](https://agentclientprotocol.com) (ACP), permettendoti di usarlo direttamente in editor e IDE compatibili.
...
Per usare MrCodeTol via ACP, configura l'editor in modo che esegua il comando `opencode acp`.

Il comando avvia MrCodeTol come sottoprocesso compatibile con ACP che comunica con l'editor via JSON-RPC su stdio.
...
Per aprirlo, seleziona il nuovo agente 'MrCodeTol' nel selettore agenti di AI Chat.
...
Per usare MrCodeTol come agente ACP in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), aggiungi quanto segue alla config di Neovim:
...
Questa config imposta CodeCompanion per usare MrCodeTol come agente ACP per la chat.
...
MrCodeTol funziona via ACP come nel terminale. Tutte le funzionalita sono supportate:
:::note
...
- Server MCP configurati nella tua configurazione MrCodeTol
