---
title: CLI
description: Opzioni e comandi della CLI di MrCodeTol.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI di MrCodeTol, per impostazione predefinita, avvia la [TUI](/docs/tui) quando viene eseguita senza argomenti.

```bash
opencode
```

Ma accetta anche i comandi documentati in questa pagina. Questo ti permette di interagire con MrCodeTol in modo programmatico.
...
Avvia l'interfaccia testuale di MrCodeTol (interfaccia utente da terminale).
...
La CLI di MrCodeTol include anche i seguenti comandi.

---

### agent

Gestisci gli agenti per MrCodeTol.
...
Collega un terminale a un backend server di MrCodeTol già in esecuzione avviato tramite i comandi `serve` o `web`.

```bash
opencode attach [url]
```

Questo consente di usare la TUI con un backend MrCodeTol remoto. Per esempio:
...
MrCodeTol si basa sull'elenco provider di [Models.dev](https://models.dev), quindi puoi usare `opencode auth login` per configurare le API key per qualunque provider tu voglia usare. Le credenziali vengono salvate in `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

Quando MrCodeTol si avvia, carica i provider dal file delle credenziali e, se presenti, anche eventuali key definite nell'ambiente o in un file `.env` nel progetto.
...
Usa `--refresh` per aggiornare l'elenco modelli in cache. È utile quando nuovi modelli vengono aggiunti a un provider e vuoi vederli in MrCodeTol.
...
Avvia un server MrCodeTol headless per accesso via API. Vedi le [server docs](/docs/server) per l'interfaccia HTTP completa.
...
Gestisci le sessioni MrCodeTol.
...
Elenca tutte le sessioni MrCodeTol.
...
Mostra statistiche di utilizzo token e costo per le sessioni MrCodeTol.
...
Importa i dati di sessione da un file JSON o da un URL di condivisione MrCodeTol.

```bash
opencode import <file>
```

Puoi importare da un file locale o da un URL di condivisione MrCodeTol.
...
Avvia un server MrCodeTol headless con interfaccia web.

```bash
opencode web
```

Avvia un server HTTP e apre un browser per accedere a MrCodeTol tramite interfaccia web. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `opencode`).
...
Disinstalla MrCodeTol e rimuove tutti i file correlati.
...
MrCodeTol può essere configurato tramite variabili d'ambiente.
