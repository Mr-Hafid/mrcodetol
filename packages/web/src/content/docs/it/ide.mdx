---
title: IDE
description: L'estensione MrCodeTol per VS Code, Cursor e altri IDE
---

MrCodeTol si integra con VS Code, Cursor o qualunque IDE che supporti un terminale. Basta eseguire `opencode` nel terminale per iniziare.

---

## Utilizzo

- **Avvio rapido**: usa `Cmd+Esc` (Mac) o `Ctrl+Esc` (Windows/Linux) per aprire MrCodeTol in una vista terminale divisa, oppure per mettere a fuoco una sessione esistente se e gia in esecuzione.
- **Nuova sessione**: usa `Cmd+Shift+Esc` (Mac) o `Ctrl+Shift+Esc` (Windows/Linux) per avviare una nuova sessione di MrCodeTol, anche se ne esiste gia una aperta. Puoi anche cliccare il pulsante MrCodeTol nell'interfaccia.
- **Consapevolezza del contesto**: condividi automaticamente con MrCodeTol la selezione corrente o la scheda attiva.
...
Per installare MrCodeTol su VS Code e fork popolari come Cursor, Windsurf, VSCodium:
...
Cerca **MrCodeTol** nel Marketplace delle estensioni e clicca **Install**.
