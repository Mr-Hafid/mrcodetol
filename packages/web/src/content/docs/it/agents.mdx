...
In MrCodeTol esistono due tipi di agenti: agenti primari e subagenti.
...
MrCodeTol include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
MrCodeTol include tre subagenti integrati: **General**, **Explore** e **Scout**. Li vediamo sotto.

---

## Integrati

MrCodeTol include due agenti primari integrati e tre subagenti integrati.
...
Un agente in sola lettura per la ricerca su documentazione esterna e dipendenze. Usalo quando devi clonare il repository di una dipendenza nella cache gestita di MrCodeTol, ispezionare il codice sorgente di una libreria o confrontare il codice locale con implementazioni upstream senza modificare il tuo workspace.
...
Se non specifichi una temperature, MrCodeTol usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di MrCodeTol sia per la config specifica del progetto.
...
L'ID modello nella configurazione MrCodeTol usa il formato `provider/model-id`. Per esempio, se stai usando [MrCodeTol Zen](/docs/zen), useresti `opencode/gpt-5.1-codex` per GPT 5.1 Codex.
