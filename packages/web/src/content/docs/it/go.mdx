...
MrCodeTol Go è un abbonamento a basso costo — **5 $ per il primo mese**, poi **10 $/mese** — che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

Go funziona come qualsiasi altro provider in MrCodeTol. Ti abboni a MrCodeTol Go e
ottieni la tua chiave API. È **completamente facoltativo** e non hai bisogno di usarlo per
utilizzare MrCodeTol.
...
MrCodeTol Go ti dà accesso a questi modelli a **5 $ per il primo mese**, poi a **10 $/mese**.

---

## Come funziona

MrCodeTol Go funziona come qualsiasi altro provider in MrCodeTol.

1. Accedi a **<a href={console}>MrCodeTol Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `MrCodeTol Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a MrCodeTol Go.
:::
...
MrCodeTol Go include i seguenti limiti:
...
Il [model id](/docs/config/#models) nella tua MrCodeTol config
...
Abbiamo creato MrCodeTol Go per:
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con MrCodeTol.
