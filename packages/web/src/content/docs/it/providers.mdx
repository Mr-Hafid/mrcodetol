---
title: Provider
description: Usa qualsiasi provider LLM in MrCodeTol.
---

import config from "../../../../config.mjs"
export const console = config.console

MrCodeTol usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di MrCodeTol.
...
MrCodeTol Zen e una lista di modelli forniti dal team MrCodeTol, testati e
verificati per funzionare bene con MrCodeTol. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da MrCodeTol Zen.
:::
...
Funziona come qualsiasi altro provider in MrCodeTol ed e totalmente opzionale.
...
Per usare Amazon Bedrock con MrCodeTol:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in MrCodeTol non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
MrCodeTol usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare MrCodeTol
...
Per usare Google Vertex AI con MrCodeTol:
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) per loggare automaticamente ogni conversazione di MrCodeTol come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di MrCodeTol elencata come sessione separata.
...
:::tip
Ollama può auto-configurarsi per MrCodeTol. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/opencode) per dettagli.
:::
...
Per usare Ollama Cloud con MrCodeTol:
...
3. Copia la chiave API da usare in MrCodeTol.
...
6. **Importante**: Prima di usare modelli cloud in MrCodeTol, devi scaricare le informazioni del modello localmente:
...
MrCodeTol Zen è una lista di modelli testati e verificati forniti dal team MrCodeTol. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>MrCodeTol Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **MrCodeTol Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di MrCodeTol.
...
I campi `limit` permettono a MrCodeTol di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
