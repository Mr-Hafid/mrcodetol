---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di MrCodeTol.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

MrCodeTol fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire MrCodeTol avvia la TUI per la directory corrente.
...
Quando usi la TUI di MrCodeTol, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a MrCodeTol. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da MrCodeTol. _Alias_: `/quit`, `/q`
...
- `leader_timeout` - Controlla per quanto tempo MrCodeTol attende dopo il leader key. Default: `2000`.
