...
You can add external tools to MrCodeTol using the _Model Context Protocol_, or MCP. MrCodeTol supports both local and remote servers.
...
You can define MCP servers in your [MrCodeTol Config](https://opencode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
MrCodeTol automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, MrCodeTol will:
...
If the server requires authentication, MrCodeTol will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `opencode mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, MrCodeTol will store the tokens securely in `~/.local/share/opencode/mcp-auth.json`.
...
Your MCPs are available as tools in MrCodeTol, alongside built-in tools. So you can manage them through the MrCodeTol config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect MrCodeTol to your Sentry account.
