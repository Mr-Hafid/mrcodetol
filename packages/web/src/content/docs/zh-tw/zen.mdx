---
title: Zen
description: 由 MrCodeTol 提供的精選模型列表。
---
...
MrCodeTol Zen 是由 MrCodeTol 團隊提供、經過測試與驗證的模型清單。

Zen 的運作方式和 MrCodeTol 中的其他供應商一樣。你登入 MrCodeTol Zen 並取得
...
:::tip
我們測試了一組與 MrCodeTol 搭配良好的精選模型與供應商。
:::
...
MrCodeTol Zen 是一個 AI 閘道，讓你可以存取這些模型。

---

## 運作方式

MrCodeTol Zen 的運作方式和 MrCodeTol 中的其他供應商一樣。

1. 你登入 **<a href={console}>MrCodeTol Zen</a>**，加入帳單資訊，然後複製你的 API 金鑰。
2. 你在 TUI 中執行 `/connect` 指令，選擇 MrCodeTol Zen，然後貼上你的 API 金鑰。
...
MrCodeTol 設定中的 [模型 ID](/docs/config/#models) 會使用 `opencode/<model-id>`
...
你可能會在使用紀錄中看到 Haiku、Nano 或 Flash 等[低成本模型](/docs/config/#models)。MrCodeTol 使用這些模型產生工作階段標題。
...
- DeepSeek V4 Flash Free 在 MrCodeTol 上限時提供。團隊正在利用這段時間收集回饋並改進模型。
- MiMo-V2.5 Free 在 MrCodeTol 上限時提供。團隊正在利用這段時間收集回饋並改進模型。
- Hy3 Free 在 MrCodeTol 上限時提供。團隊正在利用這段時間收集回饋並改進模型。
- Laguna S 2.1 Free 在 MrCodeTol 上限時提供。團隊正在利用這段時間收集回饋並改進模型。
- Nemotron 3 Ultra Free 在 MrCodeTol 上限時提供。團隊正在利用這段時間收集回饋並改進模型。
- Nemotron 3.5 Lightning Free 在 MrCodeTol 上限時提供。團隊正在利用這段時間收集回饋並改進模型。
- Big Pickle 是一個隱身模型，在 MrCodeTol 上限時免費提供。團隊正在利用這段時間收集回饋並改進模型。
