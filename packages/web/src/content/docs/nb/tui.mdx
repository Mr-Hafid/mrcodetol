...
MrCodeTol gir et interaktivt terminalgrensesnitt (TUI) for å jobbe med prosjektene dine med en LLM.

Å kjøre MrCodeTol starter TUI for gjeldende katalog.
...
Når du bruker MrCodeTol TUI, kan du skrive `/` etterfulgt av et kommandonavn for raskt å utføre handlinger. For eksempel:
...
Legg til en leverandør til MrCodeTol. Lar deg velge fra tilgjengelige leverandører og legge til deres API-nøkler.
...
Avslutt MrCodeTol. _Aliaser_: `/quit`, `/q`
...
- `leader_timeout` - Styrer hvor lenge MrCodeTol venter etter leader key. Standard er `2000`.
