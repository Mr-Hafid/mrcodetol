---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i MrCodeTol.
---

import config from "../../../../config.mjs"
export const console = config.console

MrCodeTol bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i MrCodeTol-konfigurasjonen.
...
Du kan tilpasse leverandørene gjennom `provider`-delen i MrCodeTol
konfig.
...
MrCodeTol Zen er en liste over modeller levert av MrCodeTol-teamet som har vært
testet og verifisert for å fungere godt med MrCodeTol. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med MrCodeTol Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `MrCodeTol Zen` og gå til [opencode.ai/auth](https://opencode.ai/zen).
...
Det fungerer som alle andre leverandører i MrCodeTol og er helt valgfritt å bruke.

---

## MrCodeTol Go

MrCodeTol Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av MrCodeTol-teamet som har vært
testet og verifisert for å fungere godt med MrCodeTol.

1. Kjør kommandoen `/connect` i TUI, velg `MrCodeTol Go`, og gå til [opencode.ai/auth](https://opencode.ai/zen).
...
Slik bruker du Amazon Bedrock med MrCodeTol:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører MrCodeTol:
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at MrCodeTol skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom MrCodeTol-konfigurasjonen.
...
for å lagre token i MrCodeTol auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
MrCodeTol bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse MrCodeTol
...
Slik bruker du GitHub Copilot-abonnementet med MrCodeTol:
...
Slik bruker du Google Vertex AI med MrCodeTol:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører MrCodeTol.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom MrCodeTol, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk å logge hver MrCodeTol-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver MrCodeTol-samtale oppført som en separat økt.
...
Du kan konfigurere MrCodeTol for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere MrCodeTol for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere MrCodeTol for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for MrCodeTol. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/opencode) for detaljer.
:::
...
Slik bruker du Ollama Cloud med MrCodeTol:
...
3. Kopier API-nøkkelen for bruk i MrCodeTol.
...
6. **Viktig**: Før du bruker skymodeller i MrCodeTol, må du hente modellinformasjonen lokalt:
...
MrCodeTol Zen er en liste over testede og verifiserte modeller levert av MrCodeTol-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>MrCodeTol Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **MrCodeTol Zen**.

   ```txt
   /connect
   ```

3. Skriv inn MrCodeTol API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom MrCodeTol-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom MrCodeTol-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med MrCodeTol:
...
Du kan også tilpasse modeller gjennom MrCodeTol-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med MrCodeTol. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
`limit`-feltene lar MrCodeTol forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
2. For tilpassede leverandører, sjekk MrCodeTol-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i MrCodeTol-konfigurasjonen.
