...
Du kan konfigurere MrCodeTol ved å bruke en JSON konfigurasjonsfil.

---

## Format

MrCodeTol støtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
Plasser din globale MrCodeTol-konfigurasjon i `~/.config/opencode/opencode.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Når MrCodeTol starter opp, ser den etter en konfigurasjonsfil i gjeldende katalog eller går opp til nærmeste Git-katalog.
...
- `mdns` - Aktiver mDNS-tjenesteoppdagelse. Dette lar andre enheter på nettverket oppdage MrCodeTol-serveren din.
...
Du kan konfigurere leverandørene og modellene du vil bruke i MrCodeTol-konfigurasjonen gjennom alternativene `provider`, `model` og `small_model`.
...
Alternativet `small_model` konfigurerer en egen modell for lette oppgaver som tittelgenerering. Som standard prøver MrCodeTol å bruke en rimeligere modell hvis en er tilgjengelig fra leverandøren din, ellers faller den tilbake til hovedmodellen din.
...
Standardagenten må være en primæragent (ikke en underagent). Dette kan være en innebygd agent som `"build"` eller `"plan"`, eller en [egendefinert agent](/docs/agents) du har definert. Hvis den angitte agenten ikke eksisterer eller er en subagent, vil MrCodeTol falle tilbake til `"build"` med en advarsel.
...
MrCodeTol vil automatisk laste ned eventuelle nye oppdateringer når den starter opp. Du kan deaktivere dette med alternativet `autoupdate`.
...
Som standard **tillater MrCodeTol alle operasjoner** uten å kreve eksplisitt godkjenning. Du kan endre dette ved å bruke alternativet `permission`.
...
[Plugins](/docs/plugins) utvider MrCodeTol med tilpassede verktøy, kroker og integrasjoner.
...
Dette er nyttig når du vil begrense MrCodeTol til kun å bruke spesifikke leverandører i stedet for å deaktivere dem én etter én.
