---
title: Zen
description: 由 MrCodeTol 提供的精选模型列表。
---
...
MrCodeTol Zen 是由 MrCodeTol 团队提供的一组经过测试和验证的模型。

Zen 的工作方式与 MrCodeTol 中的任何其他提供商相同。你登录 MrCodeTol Zen 并获取 API 密钥。它是**完全可选的**，即使不用它，你也可以照常使用 MrCodeTol。
...
:::tip
我们测试了一组与 MrCodeTol 配合良好的精选模型和提供商。
:::
...
MrCodeTol Zen 是一个 AI 网关，让你可以访问这些模型。

---

## 工作原理

MrCodeTol Zen 的工作方式与 MrCodeTol 中的任何其他提供商相同。

1. 登录 **<a href={console}>MrCodeTol Zen</a>**，添加你的账单信息，然后复制你的 API 密钥。
2. 在 TUI 中运行 `/connect` 命令，选择 MrCodeTol Zen，然后粘贴你的 API 密钥。
...
在你的 MrCodeTol 配置中，[模型 ID](/docs/config/#models) 使用 `opencode/<model-id>` 格式。例如，对于 GPT 5.5，你需要在配置中使用 `opencode/gpt-5.5`。
...
你可能会在使用记录中看到 Haiku、Nano 或 Flash 等[低成本模型](/docs/config/#models)。MrCodeTol 使用这些模型生成会话标题。
...
- DeepSeek V4 Flash Free 目前在 MrCodeTol 上限时免费提供。团队正在利用这段时间收集反馈并改进模型。
- MiMo-V2.5 Free 目前在 MrCodeTol 上限时免费提供。团队正在利用这段时间收集反馈并改进模型。
- Hy3 Free 目前在 MrCodeTol 上限时免费提供。团队正在利用这段时间收集反馈并改进模型。
- Laguna S 2.1 Free 目前在 MrCodeTol 上限时免费提供。团队正在利用这段时间收集反馈并改进模型。
- Nemotron 3 Ultra Free 目前在 MrCodeTol 上限时免费提供。团队正在利用这段时间收集反馈并改进模型。
- Nemotron 3.5 Lightning Free 目前在 MrCodeTol 上限时免费提供。团队正在利用这段时间收集反馈并改进模型。
- Big Pickle 是一个隐身模型，目前在 MrCodeTol 上限时免费提供。团队正在利用这段时间收集反馈并改进模型。
