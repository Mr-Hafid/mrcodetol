---
title: TUI
description: Using the MrCodeTol terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

MrCodeTol provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running MrCodeTol starts the TUI for the current directory.
...
When using the MrCodeTol TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to MrCodeTol. Allows you to select from available providers and add their API keys.
...
Exit MrCodeTol. _Aliases_: `/quit`, `/q`
...
- `leader_timeout` - Controls how long MrCodeTol waits after the leader key. Defaults to `2000`.
