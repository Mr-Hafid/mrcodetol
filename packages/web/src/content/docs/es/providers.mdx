---
title: Proveedores
description: Usando cualquier proveedor LLM en MrCodeTol.
---

import config from "../../../../config.mjs"
export const console = config.console

MrCodeTol usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración MrCodeTol.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu MrCodeTol
configuración.
...
MrCodeTol Zen es una lista de modelos proporcionados por el equipo MrCodeTol que han sido
probado y verificado para funcionar bien con MrCodeTol. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con MrCodeTol Zen.
:::
...
Funciona como cualquier otro proveedor en MrCodeTol y su uso es completamente opcional.

---

## MrCodeTol Go

MrCodeTol Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de MrCodeTol que han sido
probado y verificado para funcionar bien con MrCodeTol.

1. Ejecute el comando `/connect` en TUI, seleccione `MrCodeTol Go` y diríjase a [opencode.ai/auth](https://opencode.ai/zen).
...
Para usar Amazon Bedrock con MrCodeTol:
...
:::info
El uso de su suscripción Claude Pro/Max en MrCodeTol no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
2. Elija su método de autenticación:
   ...
   3. Nombre: `MrCodeTol`, Alcances: `api`
...
MrCodeTol utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear MrCodeTol
...
Para utilizar Google Vertex AI con MrCodeTol:
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) para registrar automáticamente cada conversación MrCodeTol como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación MrCodeTol enumerada como una sesión separada.
...
:::tip
Ollama puede configurarse automáticamente para MrCodeTol. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/opencode) para obtener más detalles.
:::
...
Para usar Ollama Cloud con MrCodeTol:
...
3. Copie la clave API para usarla en MrCodeTol.
...
6. **Importante**: Antes de usar modelos en la nube en MrCodeTol, debe obtener la información del modelo localmente:
...
MrCodeTol Zen es una lista de modelos probados y verificados proporcionada por el equipo MrCodeTol. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>MrCodeTol Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **MrCodeTol Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave MrCodeTol API.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con MrCodeTol:
...
Los campos `limit` le permiten a MrCodeTol comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
