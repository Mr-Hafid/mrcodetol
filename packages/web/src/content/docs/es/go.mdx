...
MrCodeTol Go es una suscripción de bajo costo — **$5 por tu primer mes**, luego **$10/mes** — que te brinda acceso confiable a modelos abiertos de programación populares.

Go funciona como cualquier otro proveedor en MrCodeTol. Te suscribes a MrCodeTol Go y
...
MrCodeTol Go te da acceso a estos modelos por **$5 por tu primer mes**, luego **$10/mes**.

---

## Cómo funciona

MrCodeTol Go funciona como cualquier otro proveedor en MrCodeTol.

1. Inicias sesión en **<a href={console}>MrCodeTol Zen</a>**, te suscribes a Go y
   copias tu API key.
2. Ejecutas el comando `/connect` en la TUI, seleccionas `MrCodeTol Go` y pegas
   tu API key.
3. Ejecutas `/models` en la TUI para ver la lista de modelos disponibles a través de Go.

:::note
Solo un miembro por espacio de trabajo puede suscribirse a MrCodeTol Go.
:::
...
MrCodeTol Go incluye los siguientes límites:
...
El [ID del modelo](/docs/config/#models) en tu configuración de MrCodeTol
...
4. Evitar la **dependencia (lock-in)** al permitirte usar también cualquier otro proveedor con MrCodeTol.
