...
Puede agregar herramientas externas a MrCodeTol utilizando el _Model Context Protocol_ o MCP. MrCodeTol admite servidores locales y remotos.
...
Puede definir servidores MCP en su [MrCodeTol Config](https://opencode.ai/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
MrCodeTol maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, MrCodeTol:
...
Si el servidor requiere autenticación, MrCodeTol le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `opencode mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, MrCodeTol almacenará los tokens de forma segura en `~/.local/share/opencode/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en MrCodeTol, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración MrCodeTol como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar MrCodeTol a su cuenta Sentry.
