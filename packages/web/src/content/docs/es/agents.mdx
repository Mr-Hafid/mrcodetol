...
Hay dos tipos de agentes en MrCodeTol; agentes primarios y subagentes.
...
MrCodeTol viene con dos agentes principales integrados, **Build** y **Plan**. Bien
...
MrCodeTol viene con tres subagentes integrados, **General**, **Explore** y **Scout**. Veremos esto a continuación.

---

## Integrados

MrCodeTol viene con dos agentes primarios integrados y tres subagentes integrados.
...
Un agente de solo lectura para investigar documentación externa y dependencias. Úsalo cuando necesites clonar el repositorio de una dependencia en la caché administrada de MrCodeTol, inspeccionar el código fuente de una librería o contrastar el código local con implementaciones upstream sin modificar tu espacio de trabajo.
...
Si no se especifica ninguna temperatura, MrCodeTol utiliza valores predeterminados específicos del modelo; normalmente 0 para la mayoría de los modelos, 0,55 para los modelos Qwen.
...
Esta ruta es relativa a donde se encuentra el archivo de configuración. Entonces esto funciona tanto para la configuración global MrCodeTol como para la configuración específica del proyecto.
...
El ID del modelo en su configuración MrCodeTol usa el formato `provider/model-id`. Por ejemplo, si usa [MrCodeTol Zen](/docs/zen), usaría `opencode/gpt-5.1-codex` para GPT 5.1 Codex.
