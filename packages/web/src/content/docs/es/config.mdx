---
title: Configuración
description: Usando la configuración MrCodeTol JSON.
---

Puede configurar MrCodeTol usando un archivo de configuración JSON.

---

## Formato

MrCodeTol admite los formatos **JSON** y **JSONC** (JSON con comentarios).
...
Coloque su configuración global MrCodeTol en `~/.config/opencode/opencode.json`. Utilice la configuración global para las preferencias de todo el usuario, como temas, proveedores o combinaciones de teclas.
...
Cuando se inicia MrCodeTol, busca un archivo de configuración en el directorio actual o recorre hasta el directorio Git más cercano.
...
- `mdns`: habilita el descubrimiento de servicios mDNS. Esto permite que otros dispositivos en la red descubran su servidor MrCodeTol.
...
Puede configurar los proveedores y modelos que desea utilizar en su configuración MrCodeTol a través de las opciones `provider`, `model` y `small_model`.
...
La opción `small_model` configura un modelo separado para tareas livianas como la generación de títulos. De forma predeterminada, MrCodeTol intenta utilizar un modelo más económico si su proveedor tiene uno disponible; de ​​lo contrario, recurre a su modelo principal.
...
Puede configurar el tema que desea usar en su configuración MrCodeTol a través de la opción `theme`.
...
El agente predeterminado debe ser un agente principal (no un subagente). Puede ser un agente integrado como `"build"` o `"plan"`, o un [agente personalizado](/docs/agents) que haya definido. Si el agente especificado no existe o es un subagente, MrCodeTol recurrirá a `"build"` con una advertencia.
...
MrCodeTol descargará automáticamente cualquier actualización nueva cuando se inicie. Puede desactivar esto con la opción `autoupdate`.
...
Los [complementos](/docs/plugins) amplían MrCodeTol con herramientas, enlaces e integraciones personalizados.
...
Esto es útil cuando desea restringir MrCodeTol para que utilice únicamente proveedores específicos en lugar de deshabilitarlos uno por uno.
