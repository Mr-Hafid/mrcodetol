---
title: CLI
description: Opciones y comandos de la CLI de MrCodeTol.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI de MrCodeTol de forma predeterminada inicia el [TUI](/docs/tui) cuando se ejecuta sin ningún argumento.

```bash
opencode
```

Pero también acepta comandos como se documenta en esta página. Esto le permite interactuar con MrCodeTol mediante programación.
...
Inicie la interfaz de usuario del terminal MrCodeTol.
...
La CLI de MrCodeTol también tiene los siguientes comandos.

---

### agent

Administrar agentes para MrCodeTol.
...
Conecte una terminal a un servidor backend MrCodeTol que ya se esté ejecutando y iniciado mediante los comandos `serve` o `web`.

```bash
opencode attach [url]
```

Esto permite usar TUI con un backend remoto MrCodeTol. Por ejemplo:
...
MrCodeTol funciona con la lista de proveedores en [Models.dev](https://models.dev), por lo que puede usar `opencode auth login` para configurar las claves API para cualquier proveedor que desee utilizar. Esto se almacena en `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

Cuando se inicia MrCodeTol, carga los proveedores desde el archivo de credenciales. Y si hay claves definidas en sus entornos o un archivo `.env` en su proyecto.
...
Utilice el indicador `--refresh` para actualizar la lista de modelos almacenados en caché. Esto es útil cuando se han agregado nuevos modelos a un proveedor y desea verlos en MrCodeTol.
...
Inicie un servidor MrCodeTol sin cabeza para acceso API. Consulte los [documentos del servidor](/docs/server) para conocer la interfaz HTTP completa.
...
Administrar MrCodeTol sesiones.
...
Enumere todas las sesiones MrCodeTol.
...
Muestre el uso de tokens y las estadísticas de costos para sus sesiones MrCodeTol.
...
Importe datos de sesión desde un archivo JSON o una URL compartida MrCodeTol.

```bash
opencode import <file>
```

Puede importar desde un archivo local o una URL compartida MrCodeTol.
...
Inicie un servidor MrCodeTol sin cabeza con una interfaz web.

```bash
opencode web
```

Esto inicia un servidor HTTP y abre un navegador web para acceder a MrCodeTol a través de una interfaz web. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `opencode`).
...
Desinstale MrCodeTol y elimine todos los archivos relacionados.
...
La CLI de MrCodeTol toma las siguientes banderas globales.
...
MrCodeTol se puede configurar mediante variables de entorno.
