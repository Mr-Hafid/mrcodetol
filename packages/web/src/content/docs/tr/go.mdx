...
MrCodeTol Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, MrCodeTol'daki diğer sağlayıcılar gibi çalışır. MrCodeTol Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve MrCodeTol'u kullanmak için bunu kullanmanıza gerek yoktur.
...
MrCodeTol Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

MrCodeTol Go, MrCodeTol'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>MrCodeTol Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `MrCodeTol Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye MrCodeTol Go'ya abone olabilir.
:::
...
MrCodeTol Go aşağıdaki limitleri içerir:
...
MrCodeTol yapılandırmanızdaki [model id](/docs/config/#models) formatı `opencode-go/<model-id>` şeklindedir. Örneğin, Kimi K3 için yapılandırmanızda `opencode-go/kimi-k3` kullanmalısınız.
...
MrCodeTol Go'yu şu amaçlarla oluşturduk:
...
4. MrCodeTol ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
