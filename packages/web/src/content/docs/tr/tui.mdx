---
title: TUI
description: MrCodeTol terminal kullanıcı arayüzünü kullanma.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

MrCodeTol, projelerinizde LLM ile çalışmak için etkileşimli bir terminal arayüzü (TUI) sunar.

MrCodeTol'u çalıştırdığınızda mevcut dizin için TUI başlar.
...
MrCodeTol TUI kullanırken `/` ve ardından komut adını yazarak hızlı eylemler çalıştırabilirsiniz. Örneğin:
...
MrCodeTol'a bir sağlayıcı ekler. Mevcut sağlayıcıları seçip API anahtarları eklemenizi sağlar.
...
MrCodeTol'dan çıkar. _Takma adlar_: `/quit`, `/q`
...
- `leader_timeout` - MrCodeTol'un leader key sonrasında ne kadar bekleyeceğini kontrol eder. Varsayılan değer `2000`.
