---
title: Zen
description: MrCodeTol가 제공하는 엄선된 모델 목록.
---
...
MrCodeTol Zen은 MrCodeTol 팀이 제공하는, 테스트와 검증을 거친 모델 목록입니다.

Zen은 MrCodeTol의 다른 provider와 똑같이 작동합니다. MrCodeTol Zen에 로그인해 API 키를 받으면 됩니다. 이는 **완전히 선택 사항**이며, MrCodeTol를 사용하기 위해 반드시 사용할 필요는 없습니다.
...
:::tip
MrCodeTol에서 잘 작동하는 일부 모델과 provider를 직접 테스트했습니다.
:::
...
MrCodeTol Zen은 이런 모델에 접근할 수 있게 해주는 AI gateway입니다.

---

## 작동 방식

MrCodeTol Zen은 MrCodeTol의 다른 provider와 똑같이 작동합니다.

1. **<a href={console}>MrCodeTol Zen</a>**에 로그인하고, 결제 정보를 추가한 뒤, API 키를 복사합니다.
2. TUI에서 `/connect` 명령을 실행하고, MrCodeTol Zen을 선택한 다음 API 키를 붙여넣습니다.
...
MrCodeTol config에서 사용하는 [모델 ID](/docs/config/#models)는 `opencode/<model-id>` 형식입니다. 예를 들어 GPT 5.5를 사용하려면 config에서 `opencode/gpt-5.5`를 사용하면 됩니다.
...
사용 기록에서 Haiku, Nano 또는 Flash와 같은 [저비용 모델](/docs/config/#models)을 볼 수 있습니다. MrCodeTol는 이러한 모델을 사용해 세션 제목을 생성합니다.
...
- DeepSeek V4 Flash Free는 한정된 기간 동안 MrCodeTol에서 제공됩니다. 팀은 이 기간에 피드백을 수집하고 모델을 개선합니다.
- MiMo-V2.5 Free는 한정된 기간 동안 MrCodeTol에서 제공됩니다. 팀은 이 기간에 피드백을 수집하고 모델을 개선합니다.
- Hy3 Free는 한정된 기간 동안 MrCodeTol에서 제공됩니다. 팀은 이 기간에 피드백을 수집하고 모델을 개선합니다.
- Laguna S 2.1 Free는 한정된 기간 동안 MrCodeTol에서 제공됩니다. 팀은 이 기간에 피드백을 수집하고 모델을 개선합니다.
- Nemotron 3 Ultra Free는 한정된 기간 동안 MrCodeTol에서 제공됩니다. 팀은 이 기간에 피드백을 수집하고 모델을 개선합니다.
- Nemotron 3.5 Lightning Free는 한정된 기간 동안 MrCodeTol에서 제공됩니다. 팀은 이 기간에 피드백을 수집하고 모델을 개선합니다.
- Big Pickle은 한정된 기간 동안 MrCodeTol에서 무료로 제공되는 stealth model입니다. 팀은 이 기간에 피드백을 수집하고 모델을 개선합니다.
...
MrCodeTol Zen을 만든 이유는 다음과 같습니다.
...
4. 다른 코딩 에이전트와도 함께 사용할 수 있게 해 **lock-in이 없도록** 하기 위해. 또한 MrCodeTol에서 언제나 다른 provider도 사용할 수 있게 하기 위해.
