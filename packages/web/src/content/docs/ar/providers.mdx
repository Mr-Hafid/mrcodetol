---
title: المزوّدون
description: استخدام أي موفّر LLM في MrCodeTol.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم MrCodeTol ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات MrCodeTol.
...
يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات MrCodeTol.
...
MrCodeTol Zen هي قائمة نماذج يوفّرها فريق MrCodeTol وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع MrCodeTol. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع MrCodeTol Zen.
:::
...
يعمل مثل أي مزوّد آخر في MrCodeTol واستخدامه اختياري بالكامل.

---

## MrCodeTol Go

MrCodeTol Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق MrCodeTol، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع MrCodeTol.

1. شغّل الأمر `/connect` في TUI، واختر `MrCodeTol Go`، ثم انتقل إلى [opencode.ai/auth](https://opencode.ai/zen).
...
لاستخدام Amazon Bedrock مع MrCodeTol:
...
:::info
استخدام اشتراك Claude Pro/Max داخل MrCodeTol غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
:::note[ملاحظة امتثال]
يستخدم MrCodeTol نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر MrCodeTol
...
لاستخدام Google Vertex AI مع MrCodeTol:
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) لتسجيل كل محادثة MrCodeTol تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة MrCodeTol مدرجة كجلسة مستقلة.
...
:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع MrCodeTol. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/opencode) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع MrCodeTol:
...
3. انسخ مفتاح API لاستخدامه في MrCodeTol.
...
6. **مهم**: قبل استخدام نماذج السحابة في MrCodeTol، يجب سحب معلومات النموذج محليا:
...
MrCodeTol Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق MrCodeTol. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>MrCodeTol Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **MrCodeTol Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ MrCodeTol.
...
تسمح حقول `limit` لـ MrCodeTol بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
