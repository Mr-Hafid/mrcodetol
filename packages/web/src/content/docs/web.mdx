---
title: Web
description: Using MrCodeTol in your browser.
---

MrCodeTol can run as a web application in your browser, providing the same powerful AI coding experience without needing a terminal.

![MrCodeTol Web - New Session](../../assets/web/web-homepage-new-session.png)
...
This starts a local server on `127.0.0.1` with a random available port and automatically opens MrCodeTol in your default browser.
...
By default, MrCodeTol picks an available port. You can specify a port:
...
By default, the server binds to `127.0.0.1` (localhost only). To make MrCodeTol accessible on your network:

```bash
opencode web --hostname 0.0.0.0
```

When using `0.0.0.0`, MrCodeTol will display both local and network addresses:
...
Once started, the web interface provides access to your MrCodeTol sessions.
