...
There are two types of agents in MrCodeTol; primary agents and subagents.
...
MrCodeTol comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
MrCodeTol comes with three built-in subagents, **General**, **Explore**, and **Scout**. We'll look at this below.

---

## Built-in

MrCodeTol comes with two built-in primary agents and three built-in subagents.
...
A read-only agent for external docs and dependency research. Use this when you need to clone a dependency repository into MrCodeTol's managed cache, inspect library source, or cross-reference local code against upstream implementations without modifying your workspace.
...
If no temperature is specified, MrCodeTol uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global MrCodeTol config and the project specific config.
...
The model ID in your MrCodeTol config uses the format `provider/model-id`. For example, if you're using [MrCodeTol Zen](/docs/zen), you would use `opencode/gpt-5.1-codex` for GPT 5.1 Codex.
