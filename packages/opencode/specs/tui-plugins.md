...
- Author package entrypoint is `@mrcodetol/plugin/tui`.
...
- Import types from `@mrcodetol/plugin/tui`.
- `@mrcodetol/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@mrcodetol/plugin/tui"
...
When `plugin` entries exist in a writable `.opencode` dir or `OPENCODE_CONFIG_DIR`, OpenCode installs `@mrcodetol/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@mrcodetol/plugin/tui`.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@mrcodetol/plugin/tui`.
