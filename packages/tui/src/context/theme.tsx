  ...
  init: (props: { mode: "dark" | "light"; source?: ThemeSource }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "mrcodetol")
        draft.active = typeof active === "string" ? active : "mrcodetol"
        draft.ready = false
      }),
    )
    ...
    function syncCustomThemes() {
      return themes
        ...
        .catch(() => setStore("active", "mrcodetol"))
    }
      ...
      return renderer
        ...
        .then((colors: TerminalColors) => {
          if (!colors.palette[0]) {
            ...
            if (store.active === "system") setStore("active", "mrcodetol")
            return
          }
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.mrcodetol, store.mode)
    })
