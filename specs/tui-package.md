...
`packages/cli`, or `@mrcodetol/core`.
...
- Do not replace private `packages/opencode` imports with `@mrcodetol/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@mrcodetol/tui`.
...
Status: Completed. The private `@mrcodetol/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@mrcodetol/tui`.
...
- The package has no dependency on `opencode`, `@mrcodetol/cli`, or
  `@mrcodetol/core`.
...
primitives now live in `@mrcodetol/tui`. The legacy host consumes explicit
...
and build inputs through `@mrcodetol/tui/runtime`. Movable app, component,
...
`@mrcodetol/tui/config` now owns schemas, defaults, keybind resolution, the
...
canonical in `@mrcodetol/tui`. Configured references resolve through the new
...
`@mrcodetol/tui`. The legacy host injects a narrow plugin host that retains
...
public `@mrcodetol/tui` root while retaining worker/server/config/plugin and
...
- Narrow `@mrcodetol/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@mrcodetol/core`, or either
  executable package.
...
rg '@mrcodetol/core|packages/opencode|packages/cli' packages/tui
